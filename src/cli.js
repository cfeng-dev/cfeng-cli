...
commands.last = (arg) => {
    // Supports "last reboot" to show last deploy date
    if (arg && arg.toLowerCase() === "reboot") {
        // Preferred source: generated by Netlify build (deploy-info.js)
        if (window.lastDeploy) {
            return `Last site deploy: ${window.lastDeploy}`;
        }
        // Fallback: document last modified (not strictly deploy time)
        const fallback = formatDate(new Date(document.lastModified));
        return `Last site deploy (fallback): ${fallback}`;
    }
